...
void get_remote_lib( const char* lib, pid_t pid, lib_t* result )
{
    ...
    while ( fgets( line_buffer, sizeof( line_buffer ), file_maps ) )
    {
        if ( strstr( line_buffer, lib ) )
        {
            ...
            {
                memset( str_base + count_hex, '\0', str_hex_digits - count_hex );
            }
...
ptr_u_t find_remote_function( link_map_t* lm, const char* func, pid_t pid )
{
    ...
    if ( ptr_sym.p == NULL )
    {
        ERR( "couldn't find function %s in shared lib %s in current process\n", func, lm->l_name );
        goto ret;
    }
...
uint8_t create_remote_thread( ptr_u_t thread_parameter, thread_func_t remote_func, pid_t pid )
{
    return 0;
}
...
int main( int cargs, char** args )
{
    ...
    scanf( "%i", &pid );
    printf( "Please enter path of the shared library you want to inject\n" );
    scanf( "%s", g_dllPath );

    ptr_u_t ptr_remote_dlopen = get_remote_function( "libdl", "dlopen", pid );
